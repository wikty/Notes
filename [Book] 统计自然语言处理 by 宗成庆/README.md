...
![](SRL-bilingual-example.png)

![](SRL-bilingual-align-example.png)
...
了研究。Schank and Abelson（1977）首先提出了著名的概念依存
...
衔接关系。Halliday and Hasan（1980）将衔接分为五种情况：一般指代
（reference）、替换（substitution）、省略（ellipsis）、连接
（conjunction）和词汇衔接（lexical cohesion）。

目前关于篇章衔接性的研究主要体现在共指消解（coreference
resolution）和词汇衔接性研究两个方面。
...
解转化成一个二元分类问题［孔芳等，2010］。McCarthy and
Lehnert（1995）首先提出了将判断先行语的问题转换成分类问题，通过
分类器判断指代语与每个先行语候选之间是否存在指代关系的思想。这
一思想为日后指代消解的研究开辟了一条全新的道路。Soon et al.
...
给定特征，从而实现事件代词和事件名词的消解。Kong and
...
有较大意义（代表性）的句子。在此基础上，Silber and
